...
Review pages take live events over a WebSocket at `/live/:key` (`src/live-updates.js`, `attachLiveSocket` in `src/server.js`), because Chrome allows six HTTP/1.1 connections per host and a held EventSource per page left the seventh tab blank; WebSockets do not count toward that cap. Upgrades bypass express, so the upgrade handler applies the Host allowlist itself and also requires a browser `Origin` to match the server - WebSockets are not bound by CORS, so without that check any website could read a session's chat. `/events/:key` stays for pages loaded before this change (tabs left open across an upgrade) and for the page's fallback when a running server refuses the upgrade; it answers each request with what the page missed since the cursor in `Last-Event-ID` and ends, so an old tab never holds a connection. Each emitter event has one fan-out listener however many pages are open, and a page's cleanup is registered before any await - registering it after an await once leaked pages that closed mid-setup. `live.pages` counts socket pages plus polling pages seen within `LEGACY_PAGE_TTL_MS`.
