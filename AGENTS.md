...
Run the opt-in real-browser suites: `LAVISH_AXI_BROWSER_E2E=1 node --test test/layout-audit-browser.test.js test/layout-warning-inbox.browser.test.js test/live-updates.browser.test.js` (requires Chrome; the two layout suites drive it through `chrome-devtools-axi`).
...
Later CLI invocations reuse the running server when its health version matches the current CLI version. A mismatched server is replaced only when its `/health` `live` counts (waiting polls, open review pages) are both zero: shutdown closes every connection, so replacing a server with live work would cut another agent's poll (`assessServerLiveWork`). A versioned server too old to report `live` (including upstream releases) is judged by its open TCP connections from netstat or lsof, leaving out the CLI's own (`countServerConnections`); it counts as live only when those cannot be read, because keeping an idle one blocked acks until its idle timeout. Only pre-handshake servers are still replaced unconditionally, via `POST /shutdown` and then SIGTERM by port PID. The same guard applies to the local-build force restart. A no-timeout poll whose response is cut mid-body reconnects for `POLL_RECONNECT_WINDOW_MS` instead of exiting (`fetchPollJson`); a `--timeout-ms` poll does not, so it still returns within its bound.
Review pages take live events over a WebSocket at `/live/:key` (`src/live-updates.js`, `attachLiveSocket` in `src/server.js`), because Chrome allows six HTTP/1.1 connections per host and a held EventSource per page left the seventh tab blank; WebSockets do not count toward that cap. Upgrades bypass express, so the upgrade handler applies the Host allowlist itself and also requires a browser `Origin` to match the server - WebSockets are not bound by CORS, so without that check any website could read a session's chat. `/events/:key` stays for pages loaded before this change (tabs left open across an upgrade) and for the page's fallback when a running server refuses the upgrade; it answers each request with what the page missed since the cursor in `Last-Event-ID` and ends, so an old tab never holds a connection. A page that must resync gets a snapshot plus the reload and receipt it missed, because a snapshot of state cannot show those one-off signals; the receipt survives a restart as the session's `acknowledged_feedback`. Each emitter event has one fan-out listener however many pages are open, and a page's cleanup is registered before any await - registering it after an await once leaked pages that closed mid-setup. `live.pages` counts socket pages plus polling pages seen within `LEGACY_PAGE_TTL_MS`.
...
Server self-shutdown keys off live connections (review pages and agent polls), not session status, so the next `lavish-axi <file>` re-spawns a fresh server and adopts the session from `state.json` when it is still resumable; README's Server cleanup bullet owns the user-facing idle-timeout rules.
...
8. The live channel emits `agent-presence` states: `waiting` before any poll has attached, `listening` while one is active, `working` after a poll has delivered feedback and released, and `quiet` once `working` has lasted `AGENT_QUIET_AFTER_MS` with no poll attached; the chrome allows queued feedback in every state but `working`, so an agent that goes silent cannot block sends forever. `src/agent-presence.js` owns the states and the timeout on the server, so every page, including a page opened later and an old tab on `/events/:key`, reads the same state; old chromes map the unknown `quiet` to `waiting`. An agent reply (`POST /api/:key/agent-reply`, the CLI's `--agent-reply`) concludes the working state and returns presence to `waiting`, so sends re-enable as soon as the agent answers instead of staying blocked until another poll attaches.
...
The chrome bootstraps the inbox from `initialLayoutWarnings` in the session JSON and re-syncs over the live channel, so the inbox survives a browser refresh or reconnect; only per-chrome drawer selection lives in `sessionStorage`, keyed per session so it cannot leak across artifacts.
...
Any file change emits a live `reload` event and the chrome reloads the iframe; the SDK reports scroll position with `lavish:scroll` and the chrome replays it with `lavish:restoreScroll` after the iframe loads, because the sandbox prevents direct scroll reads.
...
During version-driven shutdown, the server sends a `chrome-reload` live event so open browser chromes wait for the replacement server and then reload the whole chrome page.
...
Poll execution guidance is shared across home output, help, `next_step` responses, and the generated skill through `POLL_WAKE_PATH_RULES`: every harness gets the same completion-aware wake-path contract, while `detectInvokingAgent` adds harness-specific text: an attached-turn warning when `CODEX_SANDBOX` or `CODEX_THREAD_ID` identifies Codex, and the `run_in_background` line when `CLAUDECODE` identifies Claude. Harness-specific text never goes into `POLL_WAKE_PATH_RULES` or the static skill.
...
`src/skill.js` renders the installable Agent Skill from the same home output, telling agents to prefer an installed `lavish-axi` and fall back to `npx -y lavish-axi` only when no installed copy is found (a second version from `npx` would otherwise meet the installed one on the same server), omitting live session state, and including Hermes Agent frontmatter metadata for categorization.
...
- Every command the CLI prints with a file path goes through `shellQuote`. Agent harnesses on Windows run commands through Git Bash, which strips the backslashes from an unquoted Windows path; `test/printed-commands.test.js` runs each printed command through a real shell.
...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation. There's no in-memory cache; mutating operations are serialized through a per-store promise queue so concurrent polls and browser writes cannot overwrite one another.
