...
Later CLI invocations reuse the running server when its health version matches the current CLI version. A mismatched server is replaced only when its `/health` `live` counts (waiting polls, open review pages) are both zero: shutdown closes every connection, so replacing a server with live work would cut another agent's poll (`serverHasLiveWork`). A versioned server too old to report `live` counts as live; only pre-handshake servers are still replaced unconditionally, via `POST /shutdown` and then SIGTERM by port PID. The same guard applies to the local-build force restart. A no-timeout poll whose response is cut mid-body reconnects for `POLL_RECONNECT_WINDOW_MS` instead of exiting (`fetchPollJson`); a `--timeout-ms` poll does not, so it still returns within its bound.
