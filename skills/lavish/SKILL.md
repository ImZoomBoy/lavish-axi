...
2. Run `lavish-axi <html-file>` to open or resume a review session in the browser.
   Follow the `next_step` in its output. You may give the user the session link, but start the poll in this same turn. Do not end your turn, or reply to the user as if you are finished, before the poll is running.
