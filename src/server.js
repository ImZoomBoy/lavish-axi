...
export async function serve({
  ...
  app.post("/api/:key/feedback-ack", async (req, res, next) => {
    try {
      const feedbackId = String(req.body?.feedback_id || req.body?.feedbackId || "");
      const result = await store.acknowledgeFeedback(req.params.key, feedbackId);
      if (!result) {
        res.status(404).json({ error: "session not found" });
        return;
      }
