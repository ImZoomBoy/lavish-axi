...
The skill teaches your agent to prefer an installed `lavish-axi` (the bare bin, or the global or local npm install path) and to fall back to `npx -y lavish-axi` only when no installed copy is found, so the CLI still comes along on demand.
Preferring the installed copy keeps every command on the version already serving your review pages; `npx -y` can resolve a different one.
...
- **Agent presence** - The browser shows a notice once no agent has been checking for feedback for 20 seconds, distinguishes feedback that is queued locally, sent to Lavish, delivered to the agent, and acknowledged by the agent, keeps queued feedback for the next successful `lavish-axi poll` send even across reloads, and only blocks human sends while the agent is working on delivered feedback, from the moment a poll delivers it; the agent's reply (`--agent-reply`) concludes that work and re-enables sends. If the agent takes a batch and then goes 2 minutes without polling, sends re-open and the page says the agent has not checked back; anything sent then is kept for its next poll. The 2 minutes count from when the delivering poll let go, and restart when the agent acknowledges a batch. The next poll or agent reply ends the notice.
  The no-timeout poll always writes an immediate stderr banner so it is visibly not hung; it adds the periodic stderr wait ticks only in an interactive terminal, so when stderr is piped (as under agent harnesses) the captured output carries no tick noise. Stdout always stays reserved for the final response; a feedback batch is marked delivered when poll returns it and acknowledged only after the agent runs `lavish-axi ack <html-file> <feedback-id>` after processing it, and if the poll is interrupted before acknowledgement, re-run it because the same feedback is retained.
  ...
  Claude-specific guidance names Claude's `run_in_background` as a background facility that wakes the same agent when the poll returns.
...
- **Server version changes** - A `lavish-axi` command whose version differs from the running server's replaces the server only when nothing is connected to it.
  While a poll is waiting or a review page is open, the command uses the running server as is and says so on stderr, so another agent's poll and the user's page are not cut.
  A server too old to report its polls and pages is checked through the system's list of open connections (`netstat` on Windows, `lsof` elsewhere). If that list cannot be read, the server is kept until it stops on its own or you run `lavish-axi stop`.
  A no-timeout poll whose connection is cut anyway, such as by a server restart, reconnects and keeps waiting; it exits with an error only if the server is not back within 30 seconds.
...
| `lavish-axi <html-file>`                   | Open or resume a Lavish Editor session, with the open-time layout gate enabled by default. Unresolved layout issues from earlier in the session are preserved. Refuses to reopen a session the user explicitly ended from the browser unless `--reopen` is passed. The output leads with `poll_command`, which quotes the file path when a shell needs it, so it runs as printed in Git Bash or PowerShell; `session.status` is `launch-requested` when a browser launch was asked for (Lavish cannot confirm the page loaded) and `ready` otherwise. |
| `lavish-axi poll <html-file>`              | Long-poll until the user sends feedback or ends the session; a feedback response includes a `feedback_id` only when the server provides one, and then stays available until the agent acknowledges it; the agent acks only when a `feedback_id` is present; detected layout issues wait in the user's Layout issues inbox and arrive only when queued. Leave no-timeout polls running, or re-run them if interrupted. Codex guidance keeps polls attached to the active turn. On `status: ended`, stop polling and do not reopen uninvited.           |
| `lavish-axi ack <html-file> <feedback-id>` | Acknowledge a feedback batch after the agent has read and processed it; use the `feedback_id` returned by `poll`.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
