...
- **Agent presence** - The browser shows a notice once no agent has been checking for feedback for 20 seconds, distinguishes feedback that is queued locally, sent to Lavish, delivered to the agent, and acknowledged by the agent, keeps queued feedback for the next successful `lavish-axi poll` send even across reloads, and only blocks human sends while the agent is working on delivered feedback, from the moment a poll delivers it; the agent's reply (`--agent-reply`) concludes that work and re-enables sends.
